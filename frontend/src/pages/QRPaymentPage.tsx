...
export const QRPaymentPage: React.FC = () => {
  ...
  useEffect(() => {
    async function loadQR() {
      ...
      try {
        ...
        setQrString(res.qrString || `upi://pay?pa=prasanthbazar@icici&pn=PRASANTH+BAZAR&tr=${merchantTxnNo}&am=${amount}&cu=INR`);
      } catch (err) {
        console.error('QR Load error', err);
        setQrString(`upi://pay?pa=prasanthbazar@icici&pn=PRASANTH+BAZAR&tr=${merchantTxnNo}&am=${amount}&cu=INR`);
      } finally {
        setLoading(false);
      }
    }
    ...
    loadQR();
  }, [merchantTxnNo, amount]);

  // Polling transaction status every 4 seconds
  useEffect(() => {
  ...
  // Generate SVG QR code image via reliable public QR service
  const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(qrString || 'upi://pay')}`;
    ...
    <div className="max-w-xl mx-auto px-4 py-12 space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-950 text-white p-6 rounded-3xl border border-slate-800 shadow-xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-extrabold text-white">ICICI Bank UPI QR</h2>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 p-6 sm:p-8 shadow-sm space-y-6 text-center">
        ...
        <div className="pt-2 border-t border-slate-100">
          <button
            ...
            className="w-full py-3 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl shadow-md flex items-center justify-center gap-2 transition disabled:opacity-50"
          >
            {verifying ? (
              <>
              ...
              <>
                ...
                <span>Simulate Instant UPI Payment (Sandbox)</span>
              </>
            )}
          </button>
        </div>
